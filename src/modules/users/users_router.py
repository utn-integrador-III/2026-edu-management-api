...
def get_student_subjects(student_id: int, period: Optional[str] = Query(None)):
...
def assign_subjects(student_id: int, body: AssignSubjectsRequest):
    try:
...
def remove_subject(student_id: int, subject_id: int, period: Optional[str] = Query('2026')):
